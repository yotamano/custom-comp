...
- When using defaults to `elementProps`, do it for every subProp recursively, and don't create a one default object.
  For example, instead of doing this:
  const productAProps = elementProps?.productA || {
  ...
  do this:
  const productA = elementProps?.productA
  const productAProps = {
    productName: productA?.productName || 'iPhone 15 Pro',
    productPrice: productA?.productPrice || '$299',
    productImage: {
      width: productA?.productImage?.width || 400,
      height: productA?.productImage?.height || 400
    }
  }