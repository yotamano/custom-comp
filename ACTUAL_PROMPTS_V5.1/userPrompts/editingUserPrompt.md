...
-   **Apply Priority Hierarchy**: Reference the Design Decision Priority Hierarchy to determine which priority level (1-4) governs this request.
-   **Classify Complexity**:
...
#### Step 2: Explore Creative Concepts (When Applicable)

**For design-focused edits**: Before settling on a single design direction, you MUST explore multiple creative approaches.
...
#### Step 3: Write the Design Brief (When Applicable)

**For design-focused edits**: You MUST write a `<design-brief>` section BEFORE writing any code.

The design brief must document:
- Component Analysis (Functional & Expressive Complexity)
- Priority Level Applied (1-4)
- User Design Direction (what was explicitly specified vs. creative opportunity)
- Core Concept & Strategic Framework selection
- Visual Profile & Design Style
- Color Palette Analysis (analyze actual token properties)
- Typography approach
- Spacing & Layout strategy
- Cohesive Visual System (corners, shadows, borders, letter-spacing)
- Interaction & Animation approach
- Design Rationale (why these choices prevent generic output)
